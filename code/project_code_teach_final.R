...
yield_density_all_years <- training_merged1 %>%
  ...
  geom_density() +
  labs(title = "Density of Yield Across All Years", x = "Yield (mg/ha)", y = "Density")

ggsave(
  plot = yield_density_all_years,
...
write_csv(weather_merged, here::here("data", "weather_merged.csv"))
...
walk2(
  ...
  allplots$name,
  ~ ggsave(
    filename = here("teach_output", "png", paste0(.y, ".png")),
    plot = .x,
    height = 6,
    width = 9,
    dpi = 150
...
density_plot_train_to_test_set <- ggplot() +
  geom_density(data = merged_train, aes(x = yield_mg_ha), color = "red") +
  geom_density(data = merged_test, aes(x = yield_mg_ha), color = "blue")
...
set.seed(12345)
xgb_grid <- grid_latin_hypercube(tree_depth(), min_n(), learn_rate(), trees(), size = 5)
xgb_grid

xgb_latin_hyper_plot <- ggplot(data = xgb_grid, aes(x = tree_depth, y = min_n)) +
...
###Validation
set.seed(10)
final_fit_xgb <- last_fit(final_spec_xgb, merged_recipe, split = merged_split)
final_fit_xgb %>%
...
###Sampling using Latin-Hypercube
set.seed(123456)
lgbm_latin_hyper_plot <- grid_latin_hypercube(tree_depth(), min_n(), learn_rate(), trees(), size = 5)


ggplot(data = lgbm_latin_hyper_plot, aes(x = tree_depth, y = min_n)) +
  geom_point(aes(color = factor(learn_rate), size = trees),
             alpha = .5,
             show.legend = FALSE)
ggsave(
...
###Validation
set.seed(10)
final_fit_lgbm <- last_fit(final_spec_lgbm, merged_recipe, split = merged_split)
final_fit_lgbm %>%
  collect_predictions() #provides predictions on the test set
final_fit_lgbm %>%
  collect_metrics() %>% #gives the rmse and r2 on the test set
...
vip_xgb <- final_spec_xgb %>%
  fit(yield_mg_ha ~ ., data = bake(merged_prep, merged_train)) %>%
  vi() %>%
...
average_growing_window_testing_submission <- training_merged1 %>%
  ...
  summarise(
    day_of_year_planted = round(mean(day_of_year_planted, na.rm = TRUE)),
    day_of_year_harvested = round(mean(day_of_year_harvested, na.rm = TRUE)),
    .groups = "drop"
  ) %>%
  mutate(
...
merged_submission_final <- merged_submission %>%
  left_join(average_growing_window_testing_submission) %>%
  mutate(
...
merged_submission_final <- merged_submission %>%
  dplyr::mutate(yield_mg_ha = dplyr::if_else(is.na(yield_mg_ha), pred_yield, yield_mg_ha)) %>%
  dplyr::select(year, site, hybrid, yield_mg_ha)

write_csv(
  merged_submission_final,
