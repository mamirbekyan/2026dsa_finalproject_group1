...
```{r}
training_merged_US <- training_merged %>%
  filter(
...
ggplot() +
  geom_sf(data = site_locations) +
  geom_point(data = training_merged_US, 
             aes(x = longitude,
                 y = latitude,
                color = yield_mg_ha)
  )
...
```{r}
setdiff(testing_submission$site, training_merged_US$site)  # sites in df1 but not df2
...
training_merged_US_1 <- training_merged_US %>%
...
write_csv(training_merged_US_1,"../data/training_merged_US_1.csv")
